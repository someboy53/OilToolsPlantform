...
namespace com.Origin.Web.Framework
{
    public class BarCode
    {
        ...
        public string SaveCodeImage(string path, string text)
        {
            this.ValueFont = new System.Drawing.Font("宋体", 20);
            System.Drawing.Bitmap imgTemp = this.GetCodeImage(text, BarCode.Encode.Code128A);
            System.Web.UI.Page p = new System.Web.UI.Page();
            string serPath = p.Server.MapPath(path);
            if (!System.IO.Directory.Exists(serPath))
            {
                System.IO.Directory.CreateDirectory(serPath);
            }
            imgTemp.Save(serPath + "/" + text + ".gif", System.Drawing.Imaging.ImageFormat.Gif);

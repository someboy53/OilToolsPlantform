...
namespace com.Origin.CommonLibrary.Framework
{
    ...
    public class LogHelper
    {
        ...
        public static void SQL(string message)
        {
            ...
            if (logsql.IsInfoEnabled && message.Length>0)
            {
                logsql.Info(message);
            }
        }
        ...
        {
            string errorMsg = string.Format("【异常类型】：{0} <br>【异常信息】：{1} <br>【堆栈调用】：{2} <br>【异常源】：{3} <br>【异常方法】：{4}",
                new object[] { ex.GetType().Name, ex.Message, ex.StackTrace, ex.Source, ex.TargetSite });
